"""
运动管理应用配置模块，定义Django应用的配置参数和初始化逻辑。
...
class ExerciseConfig(AppConfig):
    ...
    name: str = "apps.exercise"
    verbose_name: str = "运动管理"
    ...
    def ready(self) -> None:
        ...
        Returns:
            None: 此方法不返回值，但会执行运动管理应用的初始化逻辑
            
        Raises:
            此方法不直接抛出异常，但会记录初始化过程中的任何错误
        """
        # 导入信号处理器，确保运动记录创建时自动处理相关逻辑
        try:
            import apps.exercise.signals  # noqa: F401
        except ImportError:
            pass