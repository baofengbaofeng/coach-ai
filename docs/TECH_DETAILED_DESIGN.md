...
#### 1.1.2 核心设计原则（豆包最佳实践）
1. **配置拆分**：开发/生产环境分离，不混用敏感信息
2. **应用集中**：所有业务 app 放在 `apps/`，结构更清晰
3. **全局公共层**：`core/` 存放跨 app 的通用代码，避免重复
4. **环境变量**：密钥、数据库信息不硬编码，更安全
5. **路由分发**：根路由只做转发，业务路由写在各自 app 内
6. **统一规范**：代码风格、返回格式、异常处理全局一致

#### 1.1.3 App内部标准结构（豆包最佳实践）
```
apps/homework/
├── __init__.py
├── apps.py           # 应用配置
├── urls.py           # 业务路由
├── views.py          # 视图层（API入口）
├── serializers.py    # 序列化/参数校验
├── services.py       # 核心业务逻辑（最重要！）
├── models.py         # 数据模型
├── filters.py        # 筛选器
└── tests.py          # 单元测试
```

**关键规范**：业务逻辑必须写在 `services.py`
- `views.py`：只做参数接收、返回响应
- `services.py`：写所有业务逻辑（解耦、易测试）
